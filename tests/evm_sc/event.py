...
class EventSCBehavior(SmartContractBehavior):
    ...
    def trigger_event(self):
        ...
        tx = contract.functions.triggerAll(
        ...
        events = contract.events.Event0.create_filter(fromBlock=block_idx, toBlock=block_idx)
        event = events.get_all_entries()[0]
        self._unittest.assertEqual(event['args'], {})

        events = contract.events.Event1.create_filter(fromBlock=block_idx, toBlock=block_idx)
        event = events.get_all_entries()[0]
        self._unittest.assertEqual(event['args']['a'], 1)

        events = contract.events.Event2.create_filter(fromBlock=block_idx, toBlock=block_idx)
        event = events.get_all_entries()[0]
        self._unittest.assertEqual(event['args']['a'], 2)
        self._unittest.assertEqual(event['args']['b'], self._kp_deployer['kp'].ss58_address)

        events = contract.events.Event3.create_filter(fromBlock=block_idx, toBlock=block_idx)
        event = events.get_all_entries()[0]
        self._unittest.assertEqual(event['args']['a'], 3)
        self._unittest.assertEqual(event['args']['b'], self._kp_deployer['kp'].ss58_address)
        self._unittest.assertEqual(event['args']['c'], "hello")

        events = contract.events.Event4.create_filter(fromBlock=block_idx, toBlock=block_idx)
        event = events.get_all_entries()[0]
        ...
        events = contract.events.EventIndex1.create_filter(fromBlock=block_idx, toBlock=block_idx)
        event = events.get_all_entries()[0]
        ...
        events = contract.events.EventIndex2.create_filter(fromBlock=block_idx, toBlock=block_idx)
        event = events.get_all_entries()[0]
        ...
        events = contract.events.EventIndex3.create_filter(fromBlock=block_idx, toBlock=block_idx)
        event = events.get_all_entries()[0]
        self._unittest.assertEqual(event['args']['a'], 33)
        self._unittest.assertEqual(event['args']['b'], self._kp_deployer['kp'].ss58_address)
        self._unittest.assertEqual(event['args']['c'].hex(), Web3.keccak(text="earth").hex())
        self._unittest.assertEqual(event['args']['d'], False)
