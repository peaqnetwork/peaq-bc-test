...
class ERC721SmartContractBehavior(SmartContractBehavior):
    ...
    def mint_and_approval_for_all_nft(self, kp_from, kp_operator, kp_to):
        ...
        new_token_id = contract.functions.mintItem(
        ...
        tx = contract.functions.mintItem(
            kp_from["kp"].ss58_address, token_uri
        ).build_transaction({
            "from": self._kp_deployer["kp"].ss58_address,
            "nonce": self._w3.eth.get_transaction_count(self._kp_deployer["kp"].ss58_address),
        ...
        tx_receipt = sign_and_submit_evm_transaction(
            tx, self._w3, self._kp_deployer["kp"]
        )
        ...
        tx = contract.functions.safeTransferFrom(
            ...
            "from": kp_operator["kp"].ss58_address,
            "nonce": self._w3.eth.get_transaction_count(kp_operator["kp"].ss58_address),
            "chainId": self._eth_chain_id
        })
        tx_receipt = sign_and_submit_evm_transaction(
            tx, self._w3, kp_operator["kp"]
        )
        self._unittest.assertEqual(
            tx_receipt["status"],
            TX_SUCCESS_STATUS,
        ...
        return {
            "mint_and_approval_for_all_nft": {
                "sender_nft": contract.functions.balanceOf(kp_from["kp"].ss58_address).call(),
                "operator_nft": contract.functions.balanceOf(kp_operator["kp"].ss58_address).call(),
                "receiver_nft": contract.functions.balanceOf(kp_to["kp"].ss58_address).call(),
    ...
    def mint_and_safe_transfer_nft(self, kp_from, kp_approval, kp_to):
        ...
        return {
            "mint_and_safe_transfer_nft": {
                "sender_nft": contract.functions.balanceOf(kp_from["kp"].ss58_address).call(),
                "approval_nft": contract.functions.balanceOf(kp_approval["kp"].ss58_address).call(),
                "receiver_nft": contract.functions.balanceOf(kp_to["kp"].ss58_address).call(),
                "owner": address_dict[owner],
                "uri": address_dict[uri_detail],
            }
    ...
    def mint_and_burn_nft(self, kp_nft):
        ...
        tx = self._batch_contract.functions.batchAll(
            [Web3.to_checksum_address(self._address), Web3.to_checksum_address(self._address)],
            [0, 0],
            [contract.encodeABI(fn_name='mintItem', args=[kp_nft["kp"].ss58_address, token_uri]),
             contract.encodeABI(fn_name='burnItem', args=[new_token_id])],
            [0, 0],
            ).build_transaction({
                'from': self._kp_deployer["kp"].ss58_address,
                'nonce': self._w3.eth.get_transaction_count(self._kp_deployer["kp"].ss58_address),
    ...
    def migration_same_behavior(self, args):
        return {
            "mint_and_burn_nft": self.mint_and_burn_nft(
                *args["mint_and_burn_nft"]
            ),
